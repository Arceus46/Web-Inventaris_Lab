        ...
        <div class="container">
            <div class="d-flex justify-content-center align-items-center" style="min-height: 100vh;">
                <div class="text-center w-100">
                    <?php
                    // Ambil data biodata user dari database berdasarkan username yang login
                    $query = "SELECT * FROM user WHERE nim = '$username' LIMIT 1";
                    $result = mysqli_query($conn, $query);
                    $userData = mysqli_fetch_assoc($result);
                    ?>

                    <h1 class="text-center">Biodata Diri</h1>
                    <div class="table-responsive mb-5" style="margin-top: 40px; margin-bottom: 40px;">
                        <table class="table w-50 mx-auto" style="font-size: 16px; background-color: #f8f9fa; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
                            <tbody>
                                <tr>
                                    <th style="width: 35%; background-color: #f8f9fa;">NIM</th>
                                    <td style="background-color: #f8f9fa; max-width: 180px; word-break: break-all;"><?php echo htmlspecialchars($userData['nim']); ?></td>
                                </tr>
                                <tr>
                                    <th style="background-color: #f8f9fa;">Nama Lengkap</th>
                                    <td style="background-color: #f8f9fa; max-width: 180px; word-break: break-all;"><?php echo htmlspecialchars($userData['nama']); ?></td>
                                </tr>
                                <tr>
                                    <th style="background-color: #f8f9fa;">No. Whatsapp</th>
                                    <td style="background-color: #f8f9fa; max-width: 180px; word-break: break-all;"><?php echo htmlspecialchars($userData['no_wa']); ?></td>
                                </tr>
                                <tr>
                                    <th style="background-color: #f8f9fa;">Alamat</th>
                                    <td style="background-color: #f8f9fa; max-width: 180px; word-break: break-all;"><?php echo htmlspecialchars($userData['Alamat']); ?></td>
                                </tr>
                                <tr>
                                    <th style="background-color: #f8f9fa;">Password</th>
                                    <td style="background-color: #f8f9fa; max-width: 180px; word-break: break-all;"><?php echo htmlspecialchars($userData['password']); ?></td>
                                </tr>
                            </tbody>
                        </table>
                        <div class="text-center mt-4">
                            <a href="update_profile.php" class="btn btn-primary" style="padding: 10px 30px; font-size: 16px;">Update</a>
                        </div>
                    </div>
                    </div>
            </div>
