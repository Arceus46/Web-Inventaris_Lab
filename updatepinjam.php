...
$simpan = mysqli_query($conn, "UPDATE peminjaman SET
    id_barang = '$id_barang',
...
?>
<?php
$sql=mysqli_query($conn,"select * from peminjaman where id_peminjaman='$_GET[id]'");
foreach ($sql as $data) {
?>
<div class="about-area-1 space">
    <div class="container">
    ...
    <input type="text" name="id_peminjaman" value="<?php echo $data['id_peminjaman']; ?>" disabled/>
</div>
    <div class="form-group">
        <label><b>ID Admin</b></label>
        <input type="text" name="id_admin" value="<?php echo $adminUsername; ?>" disabled/>
    </div>
    <div class="form-group">
        <label><b>NIM</b></label>
        <select class="form-control" name="nim" required>
            <option value="" disabled hidden>Pilih NIM</option>
            <?php
            $queryUser = mysqli_query($conn, "SELECT nim, nama FROM user");
            while ($rowUser = mysqli_fetch_assoc($queryUser)) {
                $selected = ($rowUser['nim'] == $data['nim']) ? 'selected' : '';
                echo "<option value='{$rowUser['nim']}' $selected>{$rowUser['nim']} - {$rowUser['nama']}</option>";
            }
            ?>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="date" class="form-control" id="tanggal_kembali" name="tanggal_kembali" value="<?php echo $data['tanggal_kembali']; ?>" required />
